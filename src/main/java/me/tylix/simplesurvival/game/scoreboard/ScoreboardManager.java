package me.tylix.simplesurvival.game.scoreboard;
...
public class ScoreboardManager {
    ...
    public void setScoreboard(Player player) {
        ...
        final PlayerData playerData = SimpleSurvival.INSTANCE.getChunkPlayer(player).getPlayerData();
        ...
        for (int i = 0; i < scores.size(); i++) {
            ...
            if (SimpleSurvival.INSTANCE.getChunkManager().getOwner(player.getLocation().getChunk()) == null)
                ...
                owner = Bukkit.getOfflinePlayer(SimpleSurvival.INSTANCE.getChunkManager().getOwner(player.getLocation().getChunk())).getName();
    ...
    public void updateScoreboard(Player target) {
        if (target == null)
            ...
            final PlayerData playerData = SimpleSurvival.INSTANCE.getChunkPlayer(target).getPlayerData();
            ...
            for (String s : scores) {
                ...
                String owner;
                if (SimpleSurvival.INSTANCE.getChunkManager().getOwner(target.getLocation().getChunk()) == null)
                    owner = Message.UNKNOWN_CHUNK_OWNER.getMessage();
                else {
                    owner = Bukkit.getOfflinePlayer(SimpleSurvival.INSTANCE.getChunkManager().getOwner(target.getLocation().getChunk())).getName();
                /*if (prefix.contains("$chunk_owner$"))
                    if ((prefix.length() + owner.length()) >= 16)
                        owner = owner.substring(0, 16);*/
                }
