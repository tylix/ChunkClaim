package me.tylix.simplesurvival.game.setup;
...
public class Setup {
    ...
    private void start(int time) {
        if (animation)
            task = Bukkit.getScheduler().scheduleAsyncRepeatingTask(SimpleSurvival.INSTANCE, () -> {
                for (int i = 0; i < 200; i++)
                    player.sendMessage(" ");
    ...
    public void nextStep(int id) {
        ...
        switch (id) {
            ...
            case 3:
                message = "Finished";
                start(2);
                SimpleSurvival.INSTANCE.getSetupMap().remove(player.getUniqueId());
                SimpleSurvival.INSTANCE.loadSpawnArea();
                break;
        }
    }
