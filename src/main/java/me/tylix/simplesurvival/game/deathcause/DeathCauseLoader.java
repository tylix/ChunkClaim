...
public class DeathCauseLoader {
    ...
    public DeathCauseLoader() {
        this.file = new File("plugins/SimpleSurvival/data/messages/deathCause.yml");
    ...
    public void loadDeathCauses() {
        if (cfg.getStringList("DeathCauses").isEmpty()) {
            ...
            for (EntityDamageEvent.DamageCause value : EntityDamageEvent.DamageCause.values())
                deathCauses.add(SimpleSurvival.INSTANCE.getPrettyGson().toJson(new Gson().toJsonTree(new DeathCauseData(value, new String[]{value.name()}))));
