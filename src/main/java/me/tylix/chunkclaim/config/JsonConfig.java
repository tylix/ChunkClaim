...
public class JsonConfig {
    ...
    public boolean saveConfig() {
        ...
        try (final FileWriter writer = new FileWriter(this.file)) {
            writer.write(ChunkClaim.INSTANCE.getPrettyGson().toJson(this.object));
        } finally {
            return true;
        }
    }
    ...
    public boolean reloadFromFile() {
        ...
        try (final FileReader reader = new FileReader(this.file)) {
            this.object = ChunkClaim.INSTANCE.getParser().parse(reader).getAsJsonObject();
        } catch (final IOException e) {
            return false;
        } finally {
            return true;
    ...
    public <T> T get(final String key, final Class<T> type) {
        ...
        return ChunkClaim.INSTANCE.getGson().fromJson(this.object.get(key), type);
    }

    public void set(final String key, final Object object) {
        this.object.add(key, ChunkClaim.INSTANCE.getGson().toJsonTree(object));
    }
    ...
    public void fromString(final String string) {
        this.object = ChunkClaim.INSTANCE.getParser().parse(string).getAsJsonObject();
    }
